...
RISK_KEY = {
    ...
    'I_ITFN': 'sv',    # CITFN + Semivariance
    'IV_ITFN': 'evar', # CITFN + EVaR  (new)
}

# -- Which models use ITFN? ----------------------------------------------------
_ITFN_MODELS = {'I_ITFN', 'IV_ITFN'}
...
def compute_objectives_itfn(weights, z, R_matrix, u=None, u_hat=None, rng=None,
    ...
    Includes both SV (credibilistic, ITFN-based) and EVaR (empirical) so the
    same function covers both I_ITFN (uses sv) and IV_ITFN (uses evar).
    """
    R = portfolio_returns(weights, z, R_matrix)
    ...
    return {
        ...
        'evar': compute_evar_empirical(R, p_evar),  # empirical EVaR on raw returns
...
def run_itfn_vs_evar_comparison(markets=('nse', 'nyse'), quick_test=True, verbose=True,
                                 save_plots=True):
    """Run a 3-way comparison: ITFN-SV vs ITFN-EVaR vs CTFN-EVaR.

    Models
    ------
    I_ITFN   — Coherent ITFN + Semivariance  (intuitionistic, downside-risk)
    IV_ITFN  — Coherent ITFN + EVaR          (intuitionistic, tail-risk)
    IV       — CTFN        + EVaR            (baseline, tail-risk)
    """
    if quick_test:
    ...
    if verbose:
        ...
        print(f'\n{"="*70}')
        print(f'  3-Way Comparison: ITFN-SV vs ITFN-EVaR vs CTFN-EVaR  ({mode})')
    ...
    for market in markets:
        ...
        if verbose:
            print(f'\n{"-"*70}')
            print(f'  Market: {market.upper()}  (train={R_train.shape[0]} obs, '
                  f'test={R_test.shape[0]} obs, {R_train.shape[1]} assets)')
            print(f'{"-"*70}')
            sys.stdout.flush()

        mkt_offset = 0 if market == 'nse' else 1
        results = {}  # model_key -> (best_sol, best_obs, metrics)

        for model_key, label, seed_base in [
            ('I_ITFN',  'ITFN Model I  (CITFN + SV)',   100 + mkt_offset),
            ('IV_ITFN', 'ITFN Model IV (CITFN + EVaR)', 300 + mkt_offset),
            ('IV',      'EVaR Model IV (CTFN + EVaR)',  200 + mkt_offset),
        ]:
            if verbose:
                print(f'\n  >> Running {label} ({r_max} runs x {g_max} gen)...')
                sys.stdout.flush()
            t0 = time.time()
            rep_sol, rep_obj, rep_raw, _ = optimise_model(
                R_train, model_key, r_max=r_max, pop_size=pop_size, g_max=g_max,
                verbose=verbose, base_seed=seed_base
            )
            best_sol, best_obs = _select_best_representation(rep_sol, rep_raw, model_key)
            metrics = _portfolio_test_metrics(best_sol, (best_sol > 0).astype(float), R_test)
            results[model_key] = (best_sol, best_obs, metrics)
            if verbose:
                risk_val = best_obs.get(RISK_KEY[model_key], np.nan)
                print(f'  >> {label} done in {time.time()-t0:.1f}s  '
                      f'({len(rep_sol)} reps)  '
                      f'mean={best_obs["mean"]:.4f}  '
                      f'risk={risk_val:.4f}  '
                      f'skew={best_obs["skewness"]:.4f}  '
                      f'cum={metrics["cum_return"]:.4f}  '
                      f'sharpe={metrics["sharpe"]:.4f}  '
                      f'max_dd={metrics["max_dd"]:.4f}')
                sys.stdout.flush()

        # -- Collect rows -------------------------------------------------------
        for model_key, label in [
            ('I_ITFN',  'ITFN Model I (CITFN+SV)'),
            ('IV_ITFN', 'ITFN Model IV (CITFN+EVaR)'),
            ('IV',      'EVaR Model IV (CTFN+EVaR)'),
        ]:
            _, obs, metrics = results[model_key]
            rows.append({
                'Market':          market.upper(),
                'Model':           label,
                'Mean(train)':     obs['mean'],
                'Risk(train)':     obs.get(RISK_KEY[model_key], np.nan),
                'RiskMeasure':     RISK_KEY[model_key].upper(),
                'Skew(train)':     obs['skewness'],
                'SemiKurt(train)': obs['semikurt'],
                'CumReturn(test)': metrics['cum_return'],
                'Sharpe(test)':    metrics['sharpe'],
                'MaxDD(test)':     metrics['max_dd'],
            })
